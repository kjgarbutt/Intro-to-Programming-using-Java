...
public class RandomConvert {
    ...
    */
    public static void main(String[] args) {
        Mosaic.setUse3DEffect(false);	// Removes the 3D tile effect
        Mosaic.open(ROWS, COLUMNS, SQUARE_SIZE, SQUARE_SIZE);	// opens Mosaic to CONSTANT values
        fillWithRandomColors();	// uses the subroutine to color the Mosaic
        while (Mosaic.isOpen()) {
            for (int i = 0; i < BATCH_SIZE; i++) {
                ...
                /*
                 * While open...
                 * 	For each square in a randomly chosen BATCH of squares
                 * 		select a random row
                 * 		select a random column
                 * 		utilise convertRandomNeighbour subroutine on random BATCH squares
                 */
            }
            Mosaic.delay(DELAY);	// add a millisecond delay
        }
    }
    ...
    static void fillWithRandomColors() {
        for (int row = 0; row < ROWS; row++) {
            for (int col = 0; col < COLUMNS; col++) {
                ...
                /*
                 * For a given square
                 * 	select a random number between 0-256 and assign to RGB
                 * 	set the square color using just selected colors
                 */
            }
        }
    }
    ...
    */
    static void convertRandomNeighbor(int row, int col) {
        ...
        * or col to refer to the neighbor that lies in that direction. */
        ...
        int directionNum = (int)(4*Math.random());	// directionNum = a random number 0-3
        switch (directionNum) {		// use switch to select a case based on directionNum
